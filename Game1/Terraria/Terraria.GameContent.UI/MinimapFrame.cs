...
namespace Terraria.GameContent.UI
{
	public class MinimapFrame
	{
		private class Button
		{
			...
			{
				Vector2 value = _position + parentPosition + Size * 0.5f;
				Vector2 vector = Vector2.Max(Size, new Vector2(22f, 22f)) * 0.5f;
				Vector2 vector2 = testPoint - value;
				if (Math.Abs(vector2.X) < vector.X)
				{
					return Math.Abs(vector2.Y) < vector.Y;
				}
				return false;
			}
		}
