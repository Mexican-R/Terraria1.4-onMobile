		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			...
			for (int k = num; k < num2; k++)
			{
				if (_balloons[k].Active)
				{
					...
					Vector2 value2 = new Vector2(1f / _balloons[k].Depth, 0.9f / _balloons[k].Depth);
					Vector2 vector = _balloons[k].Position;
					...
					{
						spriteBatch.Draw(_balloons[k].Texture, vector, _balloons[k].GetSourceRectangle(), color * _opacity, 0f, Vector2.Zero, value2.X * 2f, SpriteEffects.None, 0f);
					}
				}
			}
		}
