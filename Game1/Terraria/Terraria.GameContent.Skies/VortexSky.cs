...
namespace Terraria.GameContent.Skies
{
	public class VortexSky : CustomSky
	{
		...
		{
			_planetTexture = Main.Assets.Request<Texture2D>("Images/Misc/VortexSky/Planet", Main.content, (AssetRequestMode)1);
			_bgTexture = Main.Assets.Request<Texture2D>("Images/Misc/VortexSky/Background", Main.content, (AssetRequestMode)1);
			_boltTexture = Main.Assets.Request<Texture2D>("Images/Misc/VortexSky/Bolt", Main.content, (AssetRequestMode)1);
			_flashTexture = Main.Assets.Request<Texture2D>("Images/Misc/VortexSky/Flash", Main.content, (AssetRequestMode)1);
		}
		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			...
			for (int i = 0; i < _bolts.Length; i++)
			{
				...
				{
					Texture2D value5 = _boltTexture.Value;
					int life = _bolts[i].Life;
					if (life > 26 && life % 2 == 0)
					{
						value5 = _flashTexture.Value;
					}
					float scale2 = (float)life / 30f;
					spriteBatch.Draw(value5, position, null, Color.White * scale * scale2 * _fadeOpacity, 0f, Vector2.Zero, value4.X * 5f, SpriteEffects.None, 0f);
				}
			}
		}
