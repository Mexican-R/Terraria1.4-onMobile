		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			...
			for (int k = num; k < num2; k++)
			{
				if (_lanterns[k].Active)
				{
					...
					Vector2 vector = new Vector2(1f / _lanterns[k].Depth, 0.9f / _lanterns[k].Depth);
					vector *= 1.2f;
					Vector2 vector2 = _lanterns[k].Position;
					...
					{
						DrawLantern(spriteBatch, _lanterns[k], color, vector, vector2, num3);
					}
				}
			}
		}
