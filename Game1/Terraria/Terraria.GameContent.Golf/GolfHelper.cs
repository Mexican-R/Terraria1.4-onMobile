...
namespace Terraria.GameContent.Golf
{
	public static class GolfHelper
	{
		...
		public class ContactListener : IBallContactListener
		{
			...
			private static void EmitGolfballExplosion_Old(Vector2 Center)
			{
		...
		private static readonly ContactListener Listener = new ContactListener();
		...
		private static Vector2 FindVectorOnOval(Vector2 vector, Vector2 radius)
		{
		...
		private static ClubProperties GetClubPropertiesFromGolfHelper(Projectile golfHelper)
		{
			return GetClubProperties((short)Main.player[golfHelper.owner].HeldItem.type);
		}

		private static ClubProperties GetClubProperties(short itemId)
		{
		...
		public static Projectile FindGolfBallForHelper(Projectile golfHelper)
		{
			for (int i = 0; i < 1000; i++)
			{
				...
				if (projectile.active && ProjectileID.Sets.IsAGolfBall[projectile.type] && projectile.owner == golfHelper.owner && ValidateShot(projectile, Main.player[golfHelper.owner], shotVector))
				{
					return Main.projectile[i];
				}
			}
		...
		public static void DrawPredictionLine(Entity golfBall, Vector2 impactVelocity, float chargeProgress, float roughLandResistance)
		{
			...
			{
				_predictionLine = new FancyGolfPredictionLine(20);
			}
			_predictionLine.Update(golfBall, impactVelocity, roughLandResistance);
			_predictionLine.Draw(Main.Camera, Main.spriteBatch, chargeProgress);
		}
