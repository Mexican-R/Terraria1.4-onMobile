...
namespace Terraria.GameInput
{
	public class LockOnHelper
	{
		...
		public static void Update()
		{
			...
			FindMostViableTarget(LockOnMode.ThreeDS, _threeDSTarget);
			FindMostViableTarget(LockOnMode.TargetClosest, _targetClosestTarget);
		...
		private static void FindMostViableTarget(LockOnMode context, int targetVar)
		{
			targetVar = -1;
			if (UseMode == context && CanUseLockonSystem())
			{
				...
				int t2 = -1;
				Utils.Swap(t, _targets);
				Utils.Swap(t2, _pickedTarget);
		...
		public static void Draw(SpriteBatch spriteBatch)
		{
			...
			t *= num;
			Utils.Swap(t, t2);
			Color value2 = t.MultiplyRGBA(new Color(0.8f, 0.8f, 0.8f, 0.8f));
			Color value3 = t.MultiplyRGBA(new Color(0.8f, 0.8f, 0.8f, 0.8f));
			...
			for (int i = 0; i < _drawProgress.GetLength(0); i++)
			{
				...
				_drawProgress[i, 0] = MathHelper.Clamp(_drawProgress[i, 0] + ((num6 == 1) ? num2 : (0f - num2)), 0f, 1f);
				_drawProgress[i, 1] = MathHelper.Clamp(_drawProgress[i, 1] + ((num6 == 2) ? num2 : (0f - num2)), 0f, 1f);
				float num7 = _drawProgress[i, 0];
				if (num7 > 0f)
				{
					float num8 = 1f - num7 * num7;
					Vector2 pos = Main.npc[i].Top + new Vector2(0f, 0f - num5 - num8 * num4) * gravDir - Main.screenPosition;
					pos = Main.ReverseGravitySupport(pos, Main.npc[i].height);
					spriteBatch.Draw(value, pos, rectangle, value2 * num7, 0f, rectangle.Size() / 2f, new Vector2(0.58f, 1f) * scaleFactor * scaleFactor2 * (1f + num7) / 2f, SpriteEffects.None, 0f);
					spriteBatch.Draw(value, pos, rectangle2, value3 * num7 * num7, 0f, rectangle2.Size() / 2f, new Vector2(0.58f, 1f) * scaleFactor * scaleFactor2 * (1f + num7) / 2f, SpriteEffects.None, 0f);
				}
				float num9 = _drawProgress[i, 1];
				if (num9 > 0f)
				{
					...
					{
						float num12 = (float)Math.PI * 2f / num11 * (float)j + Main.GlobalTimeWrappedHourly * ((float)Math.PI * 2f) * 0.25f;
						Vector2 value5 = new Vector2(0f, num10 / 2).RotatedBy(num12);
						Vector2 pos3 = value4 + value5 - Main.screenPosition;
						pos3 = Main.ReverseGravitySupport(pos3);
						float rotation = num12 * (float)((gravDir == 1f) ? 1 : (-1)) + (float)Math.PI * (float)((gravDir == 1f) ? 1 : 0);
						spriteBatch.Draw(value, pos3, rectangle, t * num9, rotation, rectangle.Size() / 2f, new Vector2(0.58f, 1f) * scaleFactor * num3 * (1f + num9) / 2f, SpriteEffects.None, 0f);
						spriteBatch.Draw(value, pos3, rectangle2, t2 * num9 * num9, rotation, rectangle2.Size() / 2f, new Vector2(0.58f, 1f) * scaleFactor * num3 * (1f + num9) / 2f, SpriteEffects.None, 0f);
					}
				}
			}
		}
