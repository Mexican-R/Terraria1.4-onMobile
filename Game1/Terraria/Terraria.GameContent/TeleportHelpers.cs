...
namespace Terraria.GameContent
{
	public class TeleportHelpers
	{
		public static bool RequestMagicConchTeleportPosition(Player player, int crawlOffsetX, int startX, out Point landingPoint)
		{
			...
			int num4 = 0;
			int num5 = 5000;
			Vector2 value = new Vector2((float)player.width * 0.5f, player.height);
			int num6 = 40;
			...
			while (num4 < num5 && num7 < num8)
			{
				...
				if (IsInSolidTilesExtended(new Vector2(point.X * 16 + 8, point.Y * 16 + 15) - value, player.velocity, player.width, player.height, (int)player.gravDir))
				{
					if (flag)
					{
						point.Y += num;
				...
				flag = false;
				if (!IsInSolidTilesExtended(new Vector2(point.X * 16 + 8, point.Y * 16 + 15 + 16) - value, player.velocity, player.width, player.height, (int)player.gravDir) && !flag3 && (double)point.Y < Main.worldSurface)
				{
					point.Y += num;
					continue;
				}
