...
namespace Terraria.GameContent
{
	public class AmbientWindSystem
	{
		...
		private void SpawnAirborneCloud(int x, int y)
		{
			...
			Vector2 value2 = new Point(x, y).ToWorldCoordinates();
			...
			for (int i = 0; i < num; i++)
			{
				...
				Vector2 value3 = _random.NextVector2Circular(4f, 4f);
				int type = 1091 + _random.Next(2) * 2;
				float scaleFactor = 1.4f;
				float num7 = num2 + _random.NextFloat() * num3;
				float num8 = num6 + num5;
				Vector2 value4 = Vector2.UnitX.RotatedBy(num8) * scaleFactor;
