		...
		public static void TryGoingThroughPortals(Entity ent)
		{
			...
			for (int i = 0; i < FoundPortals.GetLength(0); i++)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					...
					if (!Collision.CheckAABBvLineCollision(ent.position + ent.velocity, ent.Size, start, end, 2f, collisionPoint))
					{
						continue;
					}
					...
					if (ent is Player)
					{
						...
						if (Main.netMode == 1)
						{
							NetMessage.SendData(96, -1, -1, null, player.whoAmI, vector.X, vector.Y, num3);
					...
					else if (ent is NPC)
					{
						...
						if (Main.netMode == 2)
						{
							NetMessage.SendData(100, -1, -1, null, nPC.whoAmI, vector.X, vector.Y, num3);
