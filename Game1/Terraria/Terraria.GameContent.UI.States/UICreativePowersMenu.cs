...
namespace Terraria.GameContent.UI.States
{
	public class UICreativePowersMenu : UIState
	{
		...
		private List<UIElement> CreateMainPowerStrip()
		{
			...
			GroupOptionButton<int> groupOptionButton6 = CreateSubcategoryButton<CreativePowers.DifficultySliderPower>(request, 1, "strip 1", 5, 0, mainCategory.Buttons, mainCategory.Sliders);
		...
		private static void CategoryButton_OnUpdate_DisplayTooltips(UIElement affectedElement, string categoryNameKey)
		{
			...
			if (affectedElement.IsMouseHovering)
			{
				...
				CreativePowersHelper.AddDescriptionIfNeeded(originalText, categoryNameKey);
		...
		private List<UIElement> CreateTimePowerStrip()
		{
			...
			GroupOptionButton<int> groupOptionButton = CreateSubcategoryButton<CreativePowers.ModifyTimeRate>(request, 2, "strip 2", 1, 0, timeCategory.Buttons, timeCategory.Sliders);
		...
		private List<UIElement> CreatePersonalPowerStrip()
		{
			...
			GroupOptionButton<int> groupOptionButton = CreateSubcategoryButton<CreativePowers.SpawnRateSliderPerPlayerPower>(request, 2, "strip 2", 1, 0, personalCategory.Buttons, personalCategory.Sliders);
		...
		private List<UIElement> CreateWeatherPowerStrip()
		{
			...
			creativePowerUIElementRequestInfo.PreferredButtonHeight = 40;
			CreativePowerUIElementRequestInfo request = creativePowerUIElementRequestInfo;
			GroupOptionButton<int> groupOptionButton = CreateSubcategoryButton<CreativePowers.ModifyWindDirectionAndStrength>(request, 2, "strip 2", 1, 0, weatherCategory.Buttons, weatherCategory.Sliders);
			...
			GroupOptionButton<int> groupOptionButton2 = CreateSubcategoryButton<CreativePowers.ModifyRainPower>(request, 2, "strip 2", 2, 0, weatherCategory.Buttons, weatherCategory.Sliders);
		...
		private void SetupGamepadPoints(SpriteBatch spriteBatch)
		{
			...
			{
				array = _helper.CreateUILinkStripVertical(currentID, orderedPointsByCategoryName);
			}
			if (orderedPointsByCategoryName2.Count > 0)
			{
				array2 = _helper.CreateUILinkStripVertical(currentID, orderedPointsByCategoryName2);
			}
			if (orderedPointsByCategoryName3.Count > 0)
			{
				array3 = _helper.CreateUILinkStripVertical(currentID, orderedPointsByCategoryName3);
			}
			...
			{
				array4 = _helper.CreateUILinkStripHorizontal(currentID, orderedPointsByCategoryName4);
			...
			{
				array5 = _helper.CreateUILinkPointGrid(currentID, orderedPointsByCategoryName5, _infiniteItemsWindow.GetItemsPerLine(), uILinkPoint3, array[0]);
