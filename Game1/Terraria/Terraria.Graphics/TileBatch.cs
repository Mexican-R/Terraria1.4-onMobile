...
namespace Terraria.Graphics
{
	public class TileBatch
	{
		...
		public void Draw(Texture2D texture, Vector2 position, Rectangle? sourceRectangle, VertexColors colors, Vector2 origin, float scale, SpriteEffects effects)
		{
			...
			destination.Z = scale;
			destination.W = scale;
			InternalDraw(texture, destination, scaleDestination: true, sourceRectangle, colors, origin, effects, 0f);
		}

		public void Draw(Texture2D texture, Vector4 destination, VertexColors colors)
		{
			InternalDraw(texture, destination, scaleDestination: false, _nullRectangle, colors, _vector2Zero, SpriteEffects.None, 0f);
		}
