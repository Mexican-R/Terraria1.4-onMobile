...
namespace Terraria.GameContent.Creative
{
	public class CreativePowers
	{
		public abstract class APerPlayerTogglePower : ICreativePower, IOnPlayerJoining
		{
			...
			private void button_OnUpdate(UIElement affectedElement)
			{
				...
				if (affectedElement.IsMouseHovering)
				{
					...
					CreativePowersHelper.AddDescriptionIfNeeded(originalText, _powerNameKey + "_Description");
					CreativePowersHelper.AddUnlockTextIfNeeded(originalText, GetIsUnlocked(), _powerNameKey + "_Unlock");
					CreativePowersHelper.AddPermissionTextIfNeeded(this, originalText);
		...
		public abstract class ASharedButtonPower : ICreativePower
		{
			...
			private void button_OnUpdate(UIElement affectedElement)
			{
				if (affectedElement.IsMouseHovering)
				{
					...
					CreativePowersHelper.AddDescriptionIfNeeded(originalText, _descriptionKey);
					CreativePowersHelper.AddUnlockTextIfNeeded(originalText, GetIsUnlocked(), _powerNameKey + "_Unlock");
					CreativePowersHelper.AddPermissionTextIfNeeded(this, originalText);
					Main.instance.MouseTextNoOverride(originalText, 0, 0);
				}
		...
		public abstract class ASharedTogglePower : ICreativePower, IOnPlayerJoining
		{
			...
			private void button_OnUpdate(UIElement affectedElement)
			{
				...
				if (affectedElement.IsMouseHovering)
				{
					...
					CreativePowersHelper.AddDescriptionIfNeeded(originalText, buttonTextKey + "_Description");
					CreativePowersHelper.AddUnlockTextIfNeeded(originalText, GetIsUnlocked(), buttonTextKey + "_Unlock");
					CreativePowersHelper.AddPermissionTextIfNeeded(this, originalText);
					Main.instance.MouseTextNoOverride(originalText, 0, 0);
				}
			}
		...
		public class DifficultySliderPower : ASharedSliderPower, IPersistentPerWorldContent
		{
			...
			{
				UIImage uIImage = new UIImage(Main.Assets.Request<Texture2D>(iconImagePath, Main.content, (AssetRequestMode)1))
				{
					HAlign = 1f,
					VAlign = yAnchor,
		...
		public class SpawnRateSliderPerPlayerPower : APerPlayerSliderPower, IPersistentPerPlayerContent
		{
			...
			public void Save(Player player, BinaryWriter writer)
			{
				float value = _cachePerPlayer[player.whoAmI];
				writer.Write(value);
			}
