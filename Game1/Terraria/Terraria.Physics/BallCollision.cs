	...
	public static class BallCollision
	{
		...
		private static bool GetClosestEdgeToCircle(Vector2 position, Vector2 size, Vector2 velocity, out Vector2 collisionPoint, out Tile collisionTile)
		{
			...
			for (int i = tileBounds.Left; i < tileBounds.Right; i++)
			{
				...
				{
					if (GetCollisionPointForTile(tileEdges, i, j, vector, closestPointOut, distanceSquaredOut) && !(distanceSquaredOut >= num) && !(Vector2.Dot(velocity, vector - closestPointOut) > 0f))
					{
		...
		private static bool GetCollisionPointForTile(TileEdges edgesToTest, int x, int y, Vector2 center, Vector2 closestPointOut, float distanceSquaredOut)
		{
			...
			if (GetLeftOrRightEdge(edgesToTest, x, y, tilePosition, edge))
			{
		...
		private static bool GetSlopeEdge(TileEdges edgesToTest, Tile tile, Vector2 tilePosition, LineSegment edge)
		{
			switch (tile.slope())
			{
			case 0:
		...
		private static bool GetTopOrBottomEdge(TileEdges edgesToTest, int x, int y, Vector2 tilePosition, LineSegment edge)
		{
		...
		private static bool GetLeftOrRightEdge(TileEdges edgesToTest, int x, int y, Vector2 tilePosition, LineSegment edge)
		{
