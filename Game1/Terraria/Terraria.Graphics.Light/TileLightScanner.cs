...
namespace Terraria.Graphics.Light
{
	public class TileLightScanner
	{
		...
		{
			return GetTileMask(_world.Tiles[x, y]);
		}
		...
		public void GetTileLight(int x, int y, out Vector3 outputColor)
		{
			...
			{
				ApplySurfaceLight(tile, x, y, outputColor);
			}
			else if (y > Main.UnderworldLayer)
			{
				ApplyHellLight(tile, x, y, outputColor);
			}
			ApplyWallLight(tile, x, y, localRandom, outputColor);
			if (tile.active())
			{
				ApplyTileLight(tile, x, y, localRandom, outputColor);
			}
			ApplyLavaLight(tile, outputColor);
		}

		private static void ApplyLavaLight(Tile tile, Vector3 lightColor)
		{
		...
		private static void ApplyWallLight(Tile tile, int x, int y, FastRandom localRandom, Vector3 lightColor)
		{
			float num = 0f;
			float num2 = 0f;
			float num3 = 0f;
