...
namespace Terraria.Graphics.Light
{
	public class LegacyLighting : ILightingEngine
	{
		...
		public void AddLight(int x, int y, Vector3 color)
		{
			...
			{
				value = (_rgb ? new ColorTriplet(x2, y2, z) : new ColorTriplet((x2 + y2 + z) / 3f));
		...
		private void TryUpdatingMapWithLight()
		{
			...
			Main.mapMinX = Utils.Clamp(_requestedRectLeft, blackEdgeWidth, _world.TileColumns - blackEdgeWidth);
			Main.mapMaxX = Utils.Clamp(_requestedRectRight, blackEdgeWidth, _world.TileColumns - blackEdgeWidth);
			Main.mapMinY = Utils.Clamp(_requestedRectTop, blackEdgeWidth, _world.TileRows - blackEdgeWidth);
			Main.mapMaxY = Utils.Clamp(_requestedRectBottom, blackEdgeWidth, _world.TileRows - blackEdgeWidth);
			...
			for (int i = Main.mapMinX; i < Main.mapMaxX; i++)
			{
				...
				for (int j = Main.mapMinY; j < Main.mapMaxY; j++)
				{
					...
					float num = 0f;
					if (lightingState.R > num)
					{
						num = lightingState.R;
					}
					if (lightingState.G > num)
					{
						num = lightingState.G;
					}
					if (lightingState.B > num)
					{
						num = lightingState.B;
					}
					if (IsColorOrWhiteMode)
					...
					byte b = (byte)Math.Min(255f, num * 255f);
		...
		private void DoColors()
		{
			...
			for (int i = num; i < num2; i++)
			{
				...
				case 0:
					...
					break;
				case 1:
					_swipe.InnerLoop1Start = _minBoundArea.Left - left;
					_swipe.InnerLoop2Start = _minBoundArea.Right - left;
					_swipe.InnerLoop1End = _requestedArea.Right + RenderPhases - left;
					_swipe.InnerLoop2End = _requestedArea.Left - RenderPhases - left;
					num3 = _minBoundArea.Top - top;
					num4 = _minBoundArea.Bottom - top;
