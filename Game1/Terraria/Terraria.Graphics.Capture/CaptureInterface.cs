...
namespace Terraria.Graphics.Capture
{
	public class CaptureInterface
	{
		...
		private static void ConstraintPoints()
		{
			...
			if (EdgeAPinned)
			{
				PointWorldClamp(EdgeA, offScreenTiles);
			}
			if (EdgeBPinned)
			{
				PointWorldClamp(EdgeB, offScreenTiles);
			}
		}
		...
		private void DrawCameraLock(SpriteBatch sb)
		{
			...
			if (!(num <= 0f))
			{
				...
				ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.DeathText.Value, text, new Vector2(Main.screenWidth, Main.screenHeight) / 2f + value, Color.White * num, 0f, Vector2.Zero, Vector2.One);
				ChatManager.DrawColorCodedStringWithShadow(sb, FontAssets.DeathText.Value, text2, new Vector2(Main.screenWidth, Main.screenHeight) / 2f + value2, Color.White * num, 0f, Vector2.Zero, Vector2.One);
			}
		}
