...
namespace Terraria.GameContent.Events
{
	public class DD2Event
	{
		...
		public static void SpawnNPC(int newNPC)
		{
		}
		...
		private static void GetInvasionStatus(out int currentWave, out int requiredKillCount, out int currentKillCount, bool currentlyInCheckProgress = false)
		{
			...
			{
			case 3:
				requiredKillCount = Difficulty_3_GetRequiredWaveKills(currentWave, currentKillCount, currentlyInCheckProgress);
				break;
			case 2:
				requiredKillCount = Difficulty_2_GetRequiredWaveKills(currentWave, currentKillCount, currentlyInCheckProgress);
				break;
			default:
				requiredKillCount = Difficulty_1_GetRequiredWaveKills(currentWave, currentKillCount, currentlyInCheckProgress);
				break;
			}
		}
		...
		public static void FindArenaHitbox()
		{
			...
			for (int i = 0; i < 200; i++)
			{
				...
				if (nPC.active && (nPC.type == 549 || nPC.type == 548))
				{
					...
					if (value.X > topLeft.X)
					{
						value.X = topLeft.X;
					}
					if (value.Y > topLeft.Y)
					{
						value.Y = topLeft.Y;
					}
					topLeft = nPC.BottomRight;
					if (value2.X < topLeft.X)
					{
						value2.X = topLeft.X;
					}
					if (value2.Y < topLeft.Y)
					{
						value2.Y = topLeft.Y;
					}
				}
			}
			Vector2 vector = new Vector2(16f, 16f) * 50f;
			value -= vector;
			value2 += vector;
			Vector2 vector2 = value2 - value;
			ArenaHitbox.X = (int)value.X;
			ArenaHitbox.Y = (int)value.Y;
			ArenaHitbox.Width = (int)vector2.X;
			ArenaHitbox.Height = (int)vector2.Y;
		}
		...
		private static int Difficulty_1_GetRequiredWaveKills(int waveNumber, int currentKillCount, bool currentlyInCheckProgress)
		{
			switch (waveNumber)
			{
			case -1:
		...
		private static int Difficulty_2_GetRequiredWaveKills(int waveNumber, int currentKillCount, bool currentlyInCheckProgress)
		{
			switch (waveNumber)
			{
			case -1:
		...
		private static int Difficulty_3_GetRequiredWaveKills(int waveNumber, int currentKillCount, bool currentlyInCheckProgress)
		{
			switch (waveNumber)
			{
			case -1:
