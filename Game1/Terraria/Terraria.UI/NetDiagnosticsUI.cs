...
namespace Terraria.UI
{
	public class NetDiagnosticsUI : INetDiagnosticsUI
	{
		...
		private CounterForMessage[] _counterByMessageId = new CounterForMessage[141];
		...
		public void Draw(SpriteBatch spriteBatch)
		{
			...
			for (int j = 0; j < _counterByMessageId.Length; j++)
			{
				...
				DrawCounter(spriteBatch, _counterByMessageId[j], j.ToString(), position);
			}
			...
			foreach (KeyValuePair<int, CounterForMessage> item in _counterByModuleId)
			{
				...
				DrawCounter(spriteBatch, counter, ".." + item.Key, position);
				num4++;
			}
		}
