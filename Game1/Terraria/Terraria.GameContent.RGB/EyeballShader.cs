...
namespace Terraria.GameContent.RGB
{
	public class EyeballShader : ChromaShader
	{
		...
		private void ProcessHighDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
		{
			...
			Vector2 value = vector + _pupilOffset;
			for (int i = 0; i < fragment.Count; i++)
			{
				...
				float num2 = (float)Math.Sqrt(1f - 0.4f * vector2.Y * vector2.Y) * 5f;
				float num3 = Math.Abs(vector2.X) - num2 * (1.1f * _eyelidProgress - 0.1f);
				if (num3 > 0f)
				{
					vector3 = Vector4.Lerp(vector3, _eyelidColor, Math.Min(1f, num3 * 10f));
				}
