...
namespace Terraria.GameContent.RGB
{
	public class DD2Shader : ChromaShader
	{
		...
		private void ProcessHighDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
		{
			...
			Vector2 value = fragment.CanvasCenter;
			if ((int)quality == 0)
			{
				value = new Vector2(1.7f, 0.5f);
			}
			time *= 0.5f;
			for (int i = 0; i < fragment.Count; i++)
			{
				Vector2 canvasPositionOfIndex = fragment.GetCanvasPositionOfIndex(i);
				Vector4 value2 = new Vector4(0f, 0f, 0f, 1f);
				float num = (canvasPositionOfIndex - value).Length();
				...
				Vector4 value3 = Vector4.Lerp(_darkGlowColor, _lightGlowColor, num3 * num3);
				num3 *= MathHelper.Clamp(1f - num2, 0f, 1f) * 0.75f + 0.25f;
				value2 = Vector4.Lerp(value2, value3, num3);
				if (num < 0.5f)
				{
					float amount = 1f - MathHelper.Clamp((num - 0.5f + 0.4f) / 0.4f, 0f, 1f);
					value2 = Vector4.Lerp(value2, _lightGlowColor, amount);
				}
