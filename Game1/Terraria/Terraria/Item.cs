...
namespace Terraria
{
	public class Item : Entity
	{
		...
		{
			if (bait <= 0 && paint == 0 && type != 353 && type != 849 && type != 169 && type != 75 && type != 23 && type != 408 && type != 370 && type != 1246 && type != 154)
			{
				return !notAmmo;
			}
			return false;
		...
		public void SetDefaults5(int type)
		{
			...
			case 4060:
				...
				DefaultToRangedWeapon(728, AmmoID.FallenStar, 12, 20f, hasAutoReuse: true);
			...
			case 4702:
				...
				break;
			case 4703:
				DefaultToRangedWeapon(14, AmmoID.Bullet, 45, 7f);
				knockBack = 6.5f;
				width = 50;
				height = 14;
				UseSound = SoundID.Item36;
		...
		public void SetDefaults(int Type, bool noMatCheck = false)
		{
			...
			if (ItemID.Sets.IsDrill[type] || ItemID.Sets.IsChainsaw[type])
			{
		...
		public void UpdateItem(int i)
		{
			...
			if (!beingGrabbed)
			{
				...
				if (Main.netMode != 2 && Main.expertMode && playerIndexTheItemIsReservedFor == Main.myPlayer && type >= 71 && type <= 74)
				{
					GetPickedUpByMonsters(i);
				}
				MoveInWorld(gravity, maxFallSpeed, wetVelocity);
				if (lavaWet)
				{
					CheckLavaDeath(i);
				}
		...
		private void MoveInWorld(float gravity, float maxFallSpeed, Vector2 wetVelocity)
		{
			...
			Vector4 vector = Collision.SlopeCollision(position, base.velocity, width, height, gravity);
			position.X = vector.X;
			position.Y = vector.Y;
