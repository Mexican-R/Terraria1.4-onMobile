...
namespace Terraria
{
	public class Collision
	{
		...
		{
			float u = 0f;
			return DistFromSeg(p, a1, a2, Epsilon, u) < (double)Epsilon;
		}
		...
		public static bool CheckAABBvLineCollision2(Vector2 aabbPosition, Vector2 aabbDimensions, Vector2 lineStart, Vector2 lineEnd)
		{
			...
			{
				return CheckAABBvLineCollision(aabbPosition, aabbDimensions, lineStart, lineEnd, 0.0001f, collisionPoint);
			}
			return true;
		}
		...
		public static bool SolidTilesVersatile(int startX, int endX, int startY, int endY)
		{
			if (startX > endX)
			{
				Utils.Swap(startX, endX);
			}
			if (startY > endY)
			{
				Utils.Swap(startY, endY);
			}
		...
		public static void LaserScan(Vector2 samplingPoint, Vector2 directionUnit, float samplingWidth, float maxDistance, float[] samples)
		{
			for (int i = 0; i < samples.Length; i++)
			{
				...
				Vector2 value = samplingPoint + directionUnit.RotatedBy(1.5707963705062866) * (num - 0.5f) * samplingWidth;
