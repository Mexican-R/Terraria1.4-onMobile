...
namespace Terraria
{
	public static class Utils
	{
			...
			while (list2.Count > 0)
			{
				...
				else if (flag)
				{
					if (font.MeasureString(text2).X > (float)maxWidth)
					{
						...
						int num2 = 1;
						while (font.MeasureString(str2 + text2[num2] + "-").X <= (float)maxWidth)
						{
							str2 += text2[num2++];
						}
						str2 += "-";
		...
		{
			return string.Format("{0:" + originalFormat + "}", percent).TrimEnd('0', '%', ' ').TrimEnd('.', ' ')
				.TrimStart('0', ' ') + "%";
		}

		public static void TrimTextIfNeeded(string text, DynamicSpriteFont font, float scale, float maxWidth)
		{
			int num = 0;
		...
		public static Vector2 rotateTowards(Vector2 currentPosition, Vector2 currentVelocity, Vector2 targetPosition, float maxChange)
		{
			...
			return currentVelocity.ToRotation().AngleTowards(targetAngle, (float)Math.PI / 180f).ToRotationVector2() * scaleFactor;
		}
		...
		{
			return (float)RandomNext(seed, 24) / 16777216f;
		}
		...
		public static int RandomInt(ulong seed, int max)
		{
			if ((max & -max) == max)
			{
				return (int)((long)max * (long)RandomNext(seed, 31) >> 31);
			}
			int num;
			int num2;
			do
			{
				num = RandomNext(seed, 31);
				num2 = num % max;
			}
		...
		public static void DrawCursorSingle(SpriteBatch sb, Color color, float rot = float.NaN, float scale = 1f, Vector2 manualPosition = default(Vector2), int cursorSlot = 0, int specialMode = 0)
		{
			...
			if (flag2)
			{
				sb.Draw(TextureAssets.Cursors[cursorSlot].Value, value + value2 + Vector2.One, null, color.MultiplyRGB(new Color(0.2f, 0.2f, 0.2f, 0.5f)), rot, origin, scale * 1.1f, SpriteEffects.None, 0f);
			}
			if (flag3)
			{
