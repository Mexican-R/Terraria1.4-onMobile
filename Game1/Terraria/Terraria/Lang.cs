...
namespace Terraria
{
	public class Lang
	{
		...
		{
			if (id >= 0 && id < 145 && _emojiNameCache[id] != null)
			{
		...
		public static string LocalizedDuration(TimeSpan time, bool abbreviated, bool showAllAvailableUnits)
		{
			...
			if (time.Days > 0)
			{
				...
				arg = arg + num + (abbreviated ? (" " + Language.GetTextValue("Misc.ShortDays")) : ((num == 1) ? " day" : " days"));
			...
			if (time.Hours > 0)
			{
				...
				arg = arg + num2 + (abbreviated ? (" " + Language.GetTextValue("Misc.ShortHours")) : ((num2 == 1) ? " hour" : " hours"));
				if (!showAllAvailableUnits)
				{
					return arg;
				}
			...
			if (time.Minutes > 0)
			{
				...
				arg = arg + num3 + (abbreviated ? (" " + Language.GetTextValue("Misc.ShortMinutes")) : ((num3 == 1) ? " minute" : " minutes"));
				if (!showAllAvailableUnits)
				{
					return arg;
				}
			...
			return arg + time.Seconds + (abbreviated ? (" " + Language.GetTextValue("Misc.ShortSeconds")) : ((time.Seconds == 1) ? " second" : " seconds"));
		}
	}
}
