		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			for (int i = startItemIndex; i < endItemIndex; i++)
			{
				...
				if (base.IsMouseHovering && itemSlotHitbox.Contains(Main.MouseScreen.ToPoint()) && !PlayerInput.IgnoreMouseInterface)
				{
					...
					ItemSlot.OverrideHover(inv, context);
					ItemSlot.LeftClick(inv, context);
					ItemSlot.RightClick(inv, context);
					ItemSlot.MouseHover(inv, context);
				...
				ItemSlot.Draw(spriteBatch, inv, context, itemSlotHitbox.TopLeft());
				if (num <= 0)
				{
					break;
				}
