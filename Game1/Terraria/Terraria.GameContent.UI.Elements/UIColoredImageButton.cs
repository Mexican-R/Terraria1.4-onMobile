...
namespace Terraria.GameContent.UI.Elements
{
	public class UIColoredImageButton : UIElement
	{
		...
		public UIColoredImageButton(Asset<Texture2D> texture, bool isSmall = false)
		{
			...
			if (isSmall)
			{
				_backPanelTexture = Main.Assets.Request<Texture2D>("Images/UI/CharCreation/SmallPanel", Main.content, (AssetRequestMode)1);
			}
			else
			{
				_backPanelTexture = Main.Assets.Request<Texture2D>("Images/UI/CharCreation/CategoryPanel", Main.content, (AssetRequestMode)1);
			}
			Width.Set(_backPanelTexture.Width(), 0f);
			Height.Set(_backPanelTexture.Height(), 0f);
			_backPanelHighlightTexture = Main.Assets.Request<Texture2D>("Images/UI/CharCreation/CategoryPanelHighlight", Main.content, (AssetRequestMode)1);
			if (isSmall)
			{
				_backPanelBorderTexture = Main.Assets.Request<Texture2D>("Images/UI/CharCreation/SmallPanelBorder", Main.content, (AssetRequestMode)1);
			}
			else
			{
				_backPanelBorderTexture = Main.Assets.Request<Texture2D>("Images/UI/CharCreation/CategoryPanelBorder", Main.content, (AssetRequestMode)1);
			}
		}
