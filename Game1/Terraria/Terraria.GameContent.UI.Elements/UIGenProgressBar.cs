...
namespace Terraria.GameContent.UI.Elements
{
	public class UIGenProgressBar : UIElement
	{
		...
		public UIGenProgressBar()
		{
			if (Main.netMode != 2)
			{
				_texOuterCorrupt = Main.Assets.Request<Texture2D>("Images/UI/WorldGen/Outer_Corrupt", Main.content, (AssetRequestMode)1);
				_texOuterCrimson = Main.Assets.Request<Texture2D>("Images/UI/WorldGen/Outer_Crimson", Main.content, (AssetRequestMode)1);
				_texOuterLower = Main.Assets.Request<Texture2D>("Images/UI/WorldGen/Outer_Lower", Main.content, (AssetRequestMode)1);
			}
			Recalculate();
		}
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			if (_texOuterCorrupt.IsLoaded && _texOuterCrimson.IsLoaded && _texOuterLower.IsLoaded)
			{
				...
				DrawFilling2(spriteBatch, value + new Vector2(20f, 40f), 16, completedWidth, _longBarWidth, color, Color.Lerp(color, Color.Black, 0.5f), new Color(48, 48, 48));
				color.PackedValue = 4290947159u;
				DrawFilling2(spriteBatch, value + new Vector2(50f, 60f), 8, completedWidth2, _smallBarWidth, color, Color.Lerp(color, Color.Black, 0.5f), new Color(33, 33, 33));
