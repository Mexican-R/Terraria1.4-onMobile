...
namespace Terraria.GameContent.Tile_Entities
{
	public class TEHatRack : TileEntity
	{
		...
		public void Draw(int tileLeftX, int tileTopY)
		{
			...
			Vector2 value = new Vector2((float)tileLeftX + 1.5f, tileTopY + 4) * 16f;
			dollPlayer.direction *= -1;
			Vector2 value2 = new Vector2(-dollPlayer.width / 2, -dollPlayer.height - 6) + new Vector2(dollPlayer.direction * 14, -2f);
			...
			dollPlayer.direction *= -1;
			value2 = new Vector2(-dollPlayer.width / 2, -dollPlayer.height - 6) + new Vector2(dollPlayer.direction * 12, 16f);
			dollPlayer.position = value + value2;
			Main.PlayerRenderer.DrawPlayer(Main.Camera, dollPlayer, dollPlayer.position, 0f, dollPlayer.fullRotationOrigin);
		}
		...
		private bool TryFitting(Item[] inv, int context = 0, int slot = 0, bool justCheck = false)
		{
			...
			Utils.Swap(_items[num], inv[slot]);
			if (Main.netMode == 1)
			{
